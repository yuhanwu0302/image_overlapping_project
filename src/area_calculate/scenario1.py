...
results_df = pd.DataFrame(pd.read_csv(fr'C:\Users\Lab_205\Desktop\image_overlapping_project\src\area_calculate\each_report\7001_7005_all.csv'))
results_all[7005] = results_df
#remove row of results_all[7005]["distance"] = inf
results_all[7005] = results_all[7005][results_all[7005]["distance"]!= np.inf]
results_all[7005] = results_all[7005][results_all[7005]["size"]== 257]
results_all[7005][results_all[7005]["start_index"] > 313]
filtered_results  = results_all[7005].query(f"distance >1000 ")

results_all[7005].query(f"start_index >313 &start_index <317" )



distances = list(results_all[7005]["distance"])
min_distance_index = distances.index(min(distances))
min_size = results_all[7005]["size"][min_distance_index]
min_start_index = results_all[7005]["start_index"][min_distance_index]
min_end_index = results_all[7005]["end_index"][min_distance_index]
min_distance = distances[min_distance_index]

each_best_match[7005] = {"size": min_size, "start_index": min_start_index, "end_index": min_end_index, "distance": min_distance}

each_best_match_df = pd.DataFrame(each_best_match).T
each_best_match_df.to_csv(f'C:\\Users\\Lab_205\\Desktop\\image_overlapping_project\\dataset_output\\find_pattern\\overlapping_1\\area\\7001_best_match_47.csv', index=True)


reference_grad = pd.read_csv(fr'C:\\Users\\Lab_205\\Desktop\\image_overlapping_project\\dataset_output\\find_pattern\\overlapping_1\\contourfiles\\grad_20\\{7005}_clear_gradient.csv').values.reshape(-1, 1)
reference_start_index =int(each_best_match_df[each_best_match_df.index == 7005]["start_index"].values[0])
reference_end_index = int(each_best_match_df[each_best_match_df.index == 7005]["end_index"].values[0])
...
plt.title(fr'Reference {7005} best match part')
plt.savefig(fr'C:\Users\\Lab_205\\Desktop\\scenario1\\7001_{7005}_best_match_47.png')
plt.clf()
...
contours_data = pd.read_csv(fr'C:\Users\Lab_205\Desktop\image_overlapping_project\dataset_output\find_pattern\overlapping_1\contourfiles\{7005}_clear.csv', header=None)
...
# 將DataFrame存儲到字典中
gradient_position[7005] = df



start_grad =each_best_match[7005]["start_index"]
end_grad = each_best_match[7005]["end_index"]
start_grad_row = gradient_position[7005].query(f"grad_index == {start_grad}")
end_grad_row = gradient_position[7005].query(f"grad_index == {end_grad}")
...
cv.imwrite(fr'C:\Users\Lab_205\Desktop\scenario1\check_img\{7005}_polygon_47.png', cv.fillPoly(img, [polygon_points], (0, 255, 0)))
cv.imwrite(fr'C:\Users\Lab_205\Desktop\scenario1\check_img\{7005}_all_47.png', cv.fillPoly(img, [points], (0, 255, 0)))
...
area_dic["id"].append({7005})
area_dic["area"].append(target_area)
area_dic["distance"].append(float(min_distance))
area_dic["start_index"].append(float(min_start_index))
area_dic["end_index"].append((min_end_index))
area_df = pd.DataFrame(area_dic)
area_df.to_csv(r'C:\Users\Lab_205\Desktop\scenario1\7001_esatimate_area_47.csv', index=False)
#显示图像
